...
from transformers import set_seed, AutoConfig, AutoModel, MistralPreTrainedModel, MistralConfig, DynamicCache, \
    Cache, Gemma2PreTrainedModel, Qwen2Config, Qwen2PreTrainedModel, StaticCache
import random
import numpy as np
...
class TrainDatasetForEmbedding(Dataset):
    def __init__(
        ...
        print("load misconception_mapping data:")
        misconception_mapping_df = pd.read_csv(args.doc_data)
        self.doc_dict = {}
        for _, row in misconception_mapping_df.iterrows():
            self.doc_dict[int(row['MisconceptionId'])] = row['MisconceptionName']
    ...
    def __getitem__(self, item) -> Tuple[str, List[str]]:
        ...
        passages_ids.extend(negs)

        if self.args.passage_instruction_for_retrieval is not None:
            passages = [self.args.passage_instruction_for_retrieval + p for p in passages]
        return query, passages, passages_ids
...
class EmbedCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        passage_id = [f[2] for f in features]

        if isinstance(query[0], list):
            query = sum(query, [])
        if isinstance(passage[0], list):
            passage = sum(passage, [])
        if isinstance(passage_id[0], list):
        ...
        passage_id = torch.tensor(passage_id, dtype=torch.long)

        return {"query": q_collated, "passage": d_collated, "passage_id": passage_id}
...
class BiEncoderModel(nn.Module):
    ...
    def forward(self, query, passage, passage_id):
        ...
        if self.training:
            if self.negatives_cross_device and self.use_inbatch_neg:
                ...
                passage_id = self._dist_gather_tensor(passage_id)

            group_size = p_reps.size(0) // q_reps.size(0)
            pos_passage_ids = passage_id[::group_size]  # B,1
            ...
            if self.use_inbatch_neg:
                ...
                loss = 0.
                target = torch.zeros(1, device=scores.device, dtype=torch.long)
                cnt = 0.
                for idx in range(pos_passage_ids.size(0)):
                    pos_passage_id = pos_passage_ids[idx]
                    label_mask = pos_passage_id == passage_id
                    # 正样本
                    pos_score = torch.masked_select(scores[idx], label_mask)[0].view(-1)
                    # 负样本
                    neg_score = torch.masked_select(scores[idx], ~label_mask).view(-1)
                    # score
                    score = torch.cat([pos_score, neg_score]).contiguous().view(1, -1)
                    loss = loss + self.compute_loss(score, target)
                    cnt += 1
                loss = loss / cnt
            else:
...
def parse_args():
    ...
    parser.add_argument('--lora_path', type=str, default=None, help="lora")
